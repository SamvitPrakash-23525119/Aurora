...
export default function SideButton({weatherVariable, page, icon, top, invert, right, custom}:{weatherVariable:string, page?:string, icon:string, top:string, invert?:boolean, right?:boolean, custom?: () => void }) {
	...
	return (
        <View style={[styles.SIDE_BUTTON_CONTAINER, styles[right ? 'SIDE_BUTTON_CONTAINER_RIGHT' : 'SIDE_BUTTON_CONTAINER_LEFT'], styles[invert ? 'COLOR_INVERT' : 'COLOR']]}>
            <TouchableOpacity style={[styles.TOUCHABLE_OPACITY, styles[invert ? 'COLOR_INVERT' : 'COLOR']]} onPress={!custom ? sideMenu : custom}>
